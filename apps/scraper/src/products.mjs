...
import { readLines, writeJson } from "./io.mjs";
...
try {
  if (args.search) {
    ...
    urls.push(...await discoverFromSearch(browser, args.search, { limit, headed, skipRobots }));
  }

  const seedFile = args["seed-file"] ? path.resolve(args["seed-file"]) : (!args.search ? defaultSeed : null);
  if (seedFile) urls.push(...await readLines(seedFile));

  urls = [...new Set(urls)].slice(0, limit);
  if (!urls.length) throw new Error("No product URLs discovered. Try --seed-file with explicit Easy product URLs.");
  ...
  await context.close();
  if (!products.length) throw new Error("No products could be scraped; existing catalog was left untouched.");
